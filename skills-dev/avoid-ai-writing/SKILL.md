...
### Reveals par deux-points
 
- Deux-points dramatisant une révélation ordinaire : "Le meilleur : il apprend en une session.", "Ma partie préférée : c'est gratuit.", "Le vrai twist : personne ne le remarque.", "Ce qui compte vraiment : la constance." Cousin en registre plus posé des hameçons infomerciaux — pas de fragment interrogatif, mais le deux-points fait le même travail de mise en scène : il annonce que ce qui suit mérite l'attention, sans que le contenu ait à le mériter.
- Correction : reformuler en phrase déclarative plate. "Il apprend en une session." "C'est gratuit." Si la phrase perd son intérêt sans le deux-points, c'est le contenu qui manquait, pas la ponctuation qui manque.
- Exception : deux-points qui introduisent une vraie énumération ou une définition ("Trois techniques marchent : X, Y, Z.") sont de la ponctuation normale. Le tic est le deux-points suivi d'une révélation à une seule clause qui aurait pu être une phrase autonome.
...
- Suites de phrases très courtes qui miment un effet de gravité ou de mic-drop : "C'est tout. C'est toute l'histoire.", "Rien de plus. Vraiment rien.", "Point final. Fin de la discussion.", "Simple. Basique. Efficace." Le rédacteur mise sur le rythme staccato pour manufacturer de l'importance autour d'un point ordinaire.
- Distinct de la négation empilée ("Ce n'est pas X. Ce n'est pas Y. C'est Z.") qui utilise la répétition de la négation. Ici, c'est la brièveté extrême et répétée qui porte la charge dramatique — souvent en registre affirmatif.
- Une phrase courte isolée dans un paragraphe long marche parce qu'elle contraste. Empilées, deux ou trois phrases courtes consécutives qui redisent la même chose s'annulent et sonnent scénarisées.
- Correction : garder une seule phrase courte et couper les suivantes qui la réénoncent, ou reformuler en phrase complète. Ne garder la version fragmentée que si le rythme sert vraiment le sens (rare).
...
### Réassurance non sollicitée
 
- Phrases qui consolent le lecteur avant qu'il ait exprimé un inconfort : "Tu n'es pas seul.", "Vous n'êtes pas seul.", "Ce n'est pas ta faute.", "Tu ne l'imagines pas.", "Vous ne l'imaginez pas.", "Tu n'es pas cassé.", "C'est normal de ressentir ça.", "Tu fais déjà de ton mieux." L'IA les insère par réflexe empathique dans les contenus bien-être, santé, éducation (canine, parentale, scolaire), même quand le lecteur n'a signalé aucune détresse.
- Deux problèmes. Un : le rédacteur présume l'état émotionnel du lecteur au lieu de répondre à la question posée. Deux : c'est de la validation générique qui pourrait s'appliquer à n'importe quel lecteur — ce qui la fait sonner template plutôt que compréhension.
- Distinct du ton flatteur/sycophantic (qui valide l'intelligence du lecteur) et de la ligne plate émotionnelle (qui revendique un sentiment du rédacteur). Ici, le rédacteur assigne un sentiment *au lecteur*.
- Correction : couper la phrase. Si l'inquiétude est documentée dans le contenu ("beaucoup de propriétaires de chiens réactifs se sentent isolés"), la nommer factuellement et passer à la solution. Sinon, aller directement à l'info utile.
- Exception : dans une réponse directe à quelqu'un qui a exprimé de la détresse (email, message, réponse à un commentaire), une reconnaissance courte est appropriée. Le tic est la réassurance préemptive dans un article générique, pas la réassurance en réponse à un signal.
...
- Reveals par deux-points ("Le meilleur : il apprend")
- Fragmentation dramatique ("C'est tout. C'est toute l'histoire.")
- Réassurance non sollicitée ("Tu n'es pas seul", "Ce n'est pas ta faute")
...
| Reveals par deux-points | strict | souple (1 en hook ok) | skip | skip |
| Fragmentation dramatique | strict | souple (le staccato marche parfois) | skip | skip |
| Réassurance non sollicitée | strict | strict | souple | souple |
